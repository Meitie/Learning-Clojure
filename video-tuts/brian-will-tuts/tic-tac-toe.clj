...
; returns :x if x's win, :o if o's win, otherwise nil if no winner (not neccessary a draw)
(defn winner? [board]
  (first (filter #{:x :o} (map triple-winner? (triples board)))))

; return a sequence of 'triples' (the rows, columns, and diagonals of the board)
(defn triples [board]
  (concat
